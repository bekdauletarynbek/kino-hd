    ...
    <div class="list py-5 flex items-center relative">
      ...
      <div :class="show ? 'absolute' : ''" >
        <svg v-if="!show" class="HeaderContent__icon--39VdR SearchButton__search-icon--2cYx5 ml-5" data-tid="12e1b679"
             ...
             xmlns:xlink="http://www.w3.org/1999/xlink" @click="show = true">
          <path clip-rule="evenodd"
                d="M3 10C3 13.866 6.13401 17 10 17C11.5653 17 13.0106 16.4862 14.1764 15.6181L19.5583 21L21 19.5583L15.6181 14.1764C16.4862 13.0106 17 11.5653 17 10C17 6.13401 13.866 3 10 3C6.13401 3 3 6.13401 3 10ZM15 10C15 12.7614 12.7614 15 10 15C7.23858 15 5 12.7614 5 10C5 7.23858 7.23858 5 10 5C12.7614 5 15 7.23858 15 10Z"
                fill="#9ca3af"
                fill-rule="evenodd"></path>
        </svg>
        <div v-if="show" class="flex" tabindex="1" @blur="show=false">
          <input v-model="search" class="h-8 rounded p-2" style="width: 32rem" type="text" @input="searchMulti()">
          <svg id="Capa_1"
               class="ml-3 w-5 text-white fill-current white z-40"
               style="enable-background:new 0 0 512.001 512.001" version="1.1" viewBox="0 0 512.001 512.001" x="0px"
               xml:space="preserve"
               xmlns="http://www.w3.org/2000/svg"
               xmlns:xlink="http://www.w3.org/1999/xlink" y="0px" @click="show = false">
<g>
	<g>
        ...
        <div v-if="show" class="w-24 h-72 bg-content-bg absolute py-4 px-4 rounded z-90" style="width: 32rem; overflow-y: scroll;" tabindex="1" @blur="show=false">
          <p v-if="!data" class="pt-2 text-center text-gray-300 shadow-xl">Здесь пусто</p>
          <router-link :to="{name: item.media_type, params: {id: item.id}}" v-for="item in data" :key="item.id" class="mb-5 flex" @click.native=" show = false; $emit('select', 0);">
            <img :src="`https://image.tmdb.org/t/p/w200/${item.poster_path}`" alt="" class="rounded w-20">
            <div class="px-2">
              <p class="text-gray-200">{{ item.title || item.name }}</p>
              <span class="mt-4 text-sm" v-if="item.vote_average"
                            :class="movieStatus(item.vote_average)">{{ item.vote_average.toFixed(1) }}</span>
...
export default {
  ...
  methods: {
    async searchMulti() {
      let {data} = await ahttp.get(`/search/multi?${api}&query=${this.search}`);
